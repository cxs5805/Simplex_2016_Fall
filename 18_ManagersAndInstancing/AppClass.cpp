...
void Application::InitVariables(void)
{
	...
	m_pMesh = new MyMesh();
	//m_pMesh->GenerateTorus(0.9f, 0.4f, 7, 7, C_BROWN);
	m_pMesh->GenerateTube(3.0f, 2.6f, 7.0f, 8, C_GREEN);
...
void Application::Display(void)
{
	...
	m_pMesh->Render(m_pCamera, glm::translate(IDENTITY_M4, vector3(1,0,0)) * ToMatrix4(m_qArcBall));
	m_pMesh->Render(m_pCamera, glm::translate(IDENTITY_M4, vector3(4, 0, 0)) * ToMatrix4(m_qArcBall));
	for (uint i = 0; i < 5000; i++)
		m_pMesh->Render(m_pCamera, glm::translate(IDENTITY_M4, vector3(i, 0, 0)) * ToMatrix4(m_qArcBall));
	//*/

	// better performance, but constructing matrix every frame is bottleneck
	//for (uint i = 0; i < 5000; i++)
	//	m4List.push_back(glm::translate(IDENTITY_M4, vector3(i * 2, 0, 0)) * ToMatrix4(m_qArcBall));

	// solution
	*m_m4List[0] = ToMatrix4(m_qArcBall);

	// render rigidbody
	// by itself, m_RB->Render(m_pCamera); doesn't transform RB
